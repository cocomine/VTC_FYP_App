...
import {Dimensions, PixelRatio, Platform, SafeAreaView, StatusBar, StyleSheet, View} from 'react-native';
...
const Main = ({}) => {
    ...
    const checkPermissions = useCallback(result => {
        switch (result) {
            case RESULTS.UNAVAILABLE:
                ...
                break;
            case RESULTS.DENIED:
                request((Platform.OS === 'ios' ? PERMISSIONS.IOS.LOCATION_WHEN_IN_USE : PERMISSIONS.ANDROID.ACCESS_FINE_LOCATION)).then(
                    checkPermissions,
                );
                console.log('要求權限');
                break;
    ...
    /* 開app 檢查權限 */
    useEffect(() => {
        check((Platform.OS === 'ios' ? PERMISSIONS.IOS.LOCATION_WHEN_IN_USE : PERMISSIONS.ANDROID.ACCESS_FINE_LOCATION)).then(checkPermissions);
    }, [checkPermissions]);
